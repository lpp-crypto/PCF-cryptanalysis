...
The function $s$ expresses how the allowed circuit depth grows with the input size. For instance, if $s(n) = \log n$, \cb{Comme $\log n$ n'est pas forcement un entier, est-il correct d'avoir défini $s: \mathbb{N} \to \mathbb{N}$ ?} then the wPRF is said to be \emph{logarithmically shallow}. The notion of shallow wPRFs is relevant in contexts where parallel-time complexity is a constraint, since small-depth circuits can be computed in very few parallel steps, assuming enough computational units are available.
