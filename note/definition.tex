...
\begin{definition}[(Weak) Pseudorandom Function (\wPRF, \PRF), \cite{FOCS:GolGolMic84,FOCS:NaoRei95}]
	...
	We say that the pair $(\KeyGen,\Eval)$ is a \textbf{weak pseudorandom function (\wPRF)} if for any PPT \cb{C'est quoi PPT ?} \gc{Probabilistic Polynomial Time} adversary $\adva$ and any polynomially bounded number $Q \in \N$, it holds that %\Anote{Q is not defined}%\Mnote{Done}
		...
		\end{multline*}
\end{definition}
\cb{Expliquer le rôle du paramètre Q et le symbole 'c'.} \gc{$Q$ est le nombre maximum de samples autorisés. En effet, il faudra ajouter dans les préliminaires que $\approx_c$ indique ``calculatoirement indistinguable''}
