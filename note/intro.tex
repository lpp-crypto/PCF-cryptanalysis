...
Let $n$ and $k$ be positive integers. Let $\{F_K\}_{K \in \Z_2^k}$ be a keyed function family with $F_K : \Z_2^n \to \Z_2^n$. We say that $F$ achieves $(q,\varepsilon)$-PRF security if for any distinguisher $\distinguisher$ making at most $q$ queries, the distinguishing advantage is at most $\varepsilon$, i.e.,
\[
\left| \Pr\left[\distinguisher^{F_K} = 1 \right] - \Pr\left[\distinguisher^{\mathcal{R}} = 1 \right] \right| \leq \varepsilon,
\]
where $K \leftarrow \mathbb{Z}_2^k$ is chosen uniformly at random, and $\mathcal{R}$ is a uniformly random function from $\Z_2^n$ to $\Z_2^n$.
\end{definition}
...
Let $n$, $k$, and $q$ be positive integers. A family of functions $F_K : \Z_2^n \to \Z_2^n$, indexed by a key $K \in \Z_2^k$, is said to achieve $(q,\varepsilon)$-weak PRF security if, for every distinguisher $\distinguisher$ making at most $q$ queries on inputs $x_1, \dots, x_q \in \Z_2^n$ drawn independently and uniformly at random, we have:
\[
\left| 
\Pr\left[ 
\distinguisher(x_1, \dots, x_q, F_K(x_1), \dots, F_K(x_q)) = 1 
\right]
-
\Pr\left[ 
\distinguisher(x_1, \dots, x_q, \mathcal{R}(x_1), \dots, \mathcal{R}(x_q)) = 1 
\right]
...
In the rest of this paper, we aim to move beyond asymptotic reasoning and instead adopt a concrete perspective: for a given target security level $\lambda$, we consider a weak PRF secure if no adversary can distinguish it from a random function with success probability greater than $1/2 + \varepsilon$ in less than $2^\lambda$ operations. This includes both oracle queries and any algebraic or pre-processing steps the adversary may perform. The set of operations considered can depend on the context, but one must be clear and exact on what is being counted.
