...
In order to better understand these functions, we must first introduce their formal definition. There lies one of the main obstacles in this work: the vocabulary and notions from complexity theory and symmetric differ. Let us first describe the security notion that wsPRFs must satisfy using both languages.
...
Starting from this insight, two different definitions of ``reasonable'' have eventually solidified. The first was already present in~\cite{LubRac88}, and relies on complexity classes. The other, more common in modern symmetric, relies on precise upper bounds.
