...
Let $n$ and $k$ be positive integers. Let $\{F_K\}_{K \in \Z_2^k}$ be a keyed function family with $F_K : \Z_2^n \to \Z_2^n$. We say that $F$ \cb{c'est $F$ ou $F_K$ ici ?} achieves $(q,T,\varepsilon)$-PRF security if for any distinguisher $\distinguisher$ running in time at most $T$ and making at most $q$ queries, the distinguishing advantage is at most $\varepsilon$, i.e.,
\[
