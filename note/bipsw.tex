...
\yr{Proposition: While $p$ and $q$ can be chosen \textit{a priori} arbitrary (but different of course), the first natural choice is $p=2$ and $q=3$ as the simplest one. While the versatility of the construction permits different choices for different applications (e.g. in~\cite{TCHES:DMMS21, C:HMMRSU23}, the authors choose $p=2^{31}-1$), the concrete security, when instantiated with different $p$ and $q$ can be downgraded (or not). Therefore, dedicated cryptanalysis for different choices needs to be done.}
