...
\paragraph{Remark} The above construction can be implemented by a depth-2 $\mathsf{ACC}^0$ circuit. The first layer of this circuit consists of an $\mathsf{MOD}_2$ gate, …. \cb{Geoffroy, si tu peux compléter ce serait cool !}
...
For the first construction, the parameter $n = 384$ proposed in the original paper is now considered insecure, based on the cryptanalysis results summarized in Section~\ref{sec:bipsw_cryptanalysis}. The use of $n = 768$ was  proposed later in~\cite{AC:CDDKS24}. It is worth noting that an earlier article~\cite{EC:BCMPR24} suggested using $n = 770$. However, the choice of $n = 768$ is better suited for software implementations, as it is a multiple of 32.
...
Table~\ref{tab:parameters_bipsw} summarizes these parameters, where $N$ denotes the maximum number of queries allowed under a single key.

 \cb{Geoffroy, ça vient d'où le $N = 2^{44.5}$ que t'as donné dans ta présentation ?}
...
\midrule
Section~\ref{sec:bipsw_original} (\cite{TCC:BIPSW18}) &$n = 768$& $2^{44.5}$ & \cite{AC:CDDKS24}\\
\midrule
