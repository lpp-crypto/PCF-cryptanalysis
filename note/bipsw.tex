...
\paragraph{Remark} The above construction can be implemented by a depth-2 $\mathsf{ACC}^0$ circuit: the first layer of this circuit consists of a $\mathsf{MOD}_2$ gate and a $\MOD_3$ gate, both connected to all inputs $x_i$ such that $k_i = 1$ (the $\MOD_3$ gate is additionally connected to a ``constant-2'': it checks whether $\sum_i k_ix_i+2$ is divisible by $3$), followed by a second layer with a single two-input $\MOD_2$-gate (by inspection, $F_k(x) = 0$ if and only if $\sum_i k_ix_i$ is 0 $\bmod 2$ and $1 \bmod 3$, or $\sum_i k_ix_i$ is 1 $\bmod 2$ and 0 or 2 $ \bmod 3$).
...
The authors also propose a variant, named the \emph{reversed-moduli} or $(\mathbb{F}_3, \mathbb{F}_2)$ variant,  that produces binary output. For this variant, the key would be an element of $\mathbb{Z}_3^n$, but in practice it is seen an element of $\mathbb{Z}_2^n$. In this version, the vector $z$ is first interpreted as an element of $\mathbb{Z}_3^n$. The function output is then computed as:
\[
...
More recently,~\cite{EC:ADDG24} introduced a novel application domain for this swallow weak PRF by combining it with the TFHE scheme~\cite{JC:CGGI20}, a fully homomorphic encryption construction based on programmable bootstrapping to reduce noise. In this context, the authors target a security level of $\lambda = 128$ bits and set $n = 256$, interpreting the key as a square matrix of dimension $n$.
...
Table~\ref{tab:parameters_bipsw} summarizes these parameters, where $N$ denotes the maximum number of queries allowed under a single key.
 ...
 \cb{Geoffroy, ça vient d'où le $N = 2^{44.5}$ que t'as donné dans ta présentation ?}\gc{Bonne question...}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic} 
    \State \textbf{Input:} $K \in \mathbb{Z}_2^{m \times n}$, $x \in \mathbb{Z}_2^n$, $B \in \mathbb{Z}_2^{t \times m}$
    \State $w \leftarrow Kx \mod 6$
    \State $y \leftarrow B w \mod 2$
    \State \Return $y$
  \end{algorithmic}
\end{algorithm}
\end{minipage}
...
\cb{Le deuxième algo est faux pour le moment, je corrige ça plus tard}
