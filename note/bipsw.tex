...
One of the security arguments put forward by the designers of the above construction is that it cannot be approximated by any low-degree polynomial. This is supported by the classical results of Razborov and Smolensky~\cite{Raz87,Smo87}, which show that for two distinct primes $p$ and $q$, the $\mod{p}$ \yr{comment on enlève un espace correctement avant la fonction mod ? j'essaye pmod Mod mais ça fait des parenthèses ?} function cannot be approximated by low-depth $\mod{q}$ circuits. %\yr{Je suppose que c'est pour une taille d'espace assez grande par rapport à $p$ et $q$ sinon ça ne marche pas non ?} 
