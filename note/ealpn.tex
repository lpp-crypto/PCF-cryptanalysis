...
\begin{itemize}
	...
	\item Sample a weight-$t$ \emph{regular noise vector} $e$ as a concatenation of $t$ length-$w$ random unit vectors, that is $e$ is drawn at random from $\{1,2,4,\ldots,2^{w-1}\}^t$ \yr{the set if we represent as integers, that are those of Hamming weight 1, see line 1180 in reviews}.
...
\end{tikzpicture}
\end{center}
\yr{Write mathematically the light-red bands vector}
...
\begin{itemize}
	\item The attacker is given $M = H\cdot \Delta_{5N}$ (the parity-check matrix of the code) and produces a nonzero \emph{test vector} $v$. This step can run in unbounded time. \yr{Explain how this test vector is produced. Je suppose que c'est juste un truc aléatoire ?}
