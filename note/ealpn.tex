...
\begin{itemize}
	...
	\item Sample a weight-$t$ \emph{regular noise vector} $e$ as a concatenation of $t$ length-$w$ random unit vectors, that is $e$ is drawn at random from $\{1,2,4,\ldots,2^{w-1}\}^t$, and each integer $2^i$ is viewed as a unit vector over $\bit^w$ via its binary encoding. %\yr{the set if we represent as integers, that are those of Hamming weight 1, see line 1180 in reviews}.
...
\end{minipage}
~\\
%\yr{Write mathematically the light-red bands vector (once only)}

In the above equation, the rightmost vector is a concatenation of vectors of the form $(0,\cdots, 0, 1\cdots, 1)$ (a string of 0's followed by a string of 1's) for every block at an odd position (the 1st, 3rd, 5th, etc, blocks), and vectors of the form $(1, \cdots, 1, 0, \cdots, 0)$ (a string of 1's followed by a string of 0's) for every block at an even position.
