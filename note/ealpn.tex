...
\begin{itemize}
	...
	\item Sample a weight-$t$ \emph{regular noise vector} $e$ as a concatenation of $t$ length-$w$ random unit vectors, that is $e$ is drawn at random from $\{1,2,4,\ldots,2^{w-1}\}^t$, and each integer $2^i$ is viewed as a unit vector over $\bit^w$ via its binary encoding. 
