...
\end{equation}
where $\ell$ and $w$ are parameters that depend on the desired security level $\secpar$, and $x = (x_i,x'_i)_{i \leq \ell}$ is sampled from $([1,5N/w]\times [1,w])^\ell$. We assume that $N, w, \ell$ are chosen such that $w$ divides $5N$ and $\ell$ divides $w$.

\subsection{Security Arguments and Cryptanalysis}
%!TODO! Write about security arguments and cryptanalysis

The presentation of {\sf EALPN} in this section differs significantly from the description provided in~\cite{C:BCGIKR22}. Before we overview the security arguments for {\sf EALPN}, we explain the discrepancy (the alternative view taken in~\cite{C:BCGIKR22} is useful to understand the security rationale). The original description of {\sf EALPN} is as follows:
\begin{itemize}
	...
	\item Sample a weight-$w$ \emph{regular noise vector} $e$ as a concatenation of $w$ length-$5N/w$ random unit vectors.
...
\[\GT(x_i, k_{x'_i})\]
for all of the $\ell$ nonzero entries of the row of $H$. Hence, sampling a random row $h$ of $H$ amounts to sampling $\ell$ pairs $(x_i, x'_i) \sample [1,5N/w]\times [1,w]$
...
\subsection{Algorithmic Decription}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
  \State \textbf{Input:}  $x = ((j_1,x_1), \dots, (j_\ell,x_\ell)) \in ( \Z_{\frac{t}{\ell}}\times \Z_{w})^\ell$; $k = (k_1, \dots, k_{\ell}) \in (\Z_{w}^{\frac{t}{\ell}})^{\ell}$
    \State $r \gets 0$
\For{$i = 1$ to $\ell$}
    \If{$x_i \geq k_{j_i}$}
        \State $r \gets r + 1$
