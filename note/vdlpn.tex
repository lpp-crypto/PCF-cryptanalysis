...
The candidates introduced in section and in the next section (Section~\ref{sec:ealpn}) have been designed to be used in constructions of pseudorandom correlation functions that follow the paradigm outlined in Section~\ref{subsubsec:fss}: they belong to the class $\setF_{t,N}(\F_{2^\secpar})$ of depth-$2$ circuits with indegree-$1$ ``interval membership'' gates on the bottom and a $t$-ary linear combination gate on top, for a target number of samples $N$ and a parameter $t$ that they seek to minimize. In fact, both candidates actually belong to $\setF_{t,N}(\F_{2})$: recall that Theorem~\ref{thm:pcf-ot} uses function secret sharing for the class of functions $x \mapsto c\cdot \Eval(\msk,x)$. In constructions based on VDLPN and EALPN, $\Eval(\msk,x)$ is over $\F_2$, and only the scalar $c$ belongs to the extension field $\F_{2^\secpar}$ (where the reader can think of $\secpar = 128$).

In this section, we introduce the VDLPN design. We provide a direct description below (as a XOR of ANDs of XORs), but the latter slightly obscures the connection to the class $\setF_{t,N}(\F_{2})$. To see the connection, note that a point function (a function $f_a$ that outputs $0$ everywhere, except on $f_a(a) = 1$) is a special case of an interval function where the interval is reduced to a single point. In turn, testing for $f_a(x) = 1$ amounts to testing whether $x = a$, which can be written directly as $\prod_{i=1}^{|x|} (x_i \oplus \bar a_i)$
 (where the $\bar a_i$ denote the bits of $a$ flipped). Hence, a ``XOR of products of XORs'' is nothing more than a linear combination (over $\F_2$) of point functions (which are a special case of interval functions).

\subsection{Original Reference}

The next shallow weak PRF we present was proposed by Boyle, Couteau, Gilboa, Ishai, Kohl, and Scholl in 2020~\cite{vdlpn}. The main candidate introduced in their work, that we will refer to as {\sf VDLPN} (for \emph{variable-density learning parity with noise}), is defined as follows:
...
The candidate {\sf VDLPN}$^*$ also admits a similar low-complexity variant as {\sf VDLPN} where the triangular functions are replaced with ``shaved'' somewhat triangular functions. Denoting
...
\bottomrule
\end{tabular}
\caption{\label{tab:vdlpn-param}Parameter sets for the {\sf VDLPN} and {\sf VDLPN}$^*$ shallow wPRFs, intended as targets for cryptanalysis at the $\secpar = 128$ security level. The parameter $r$ is used only for {\sf VDLPN}$^*$.}
\end{table}

\subsection{Algorithmic Description}
The algorithmic descriptions of the {\sf VDLPN} shallow wPRF and its low-complexity variant {\sf VDLPN}$^*$ are provided in Algorithm~\ref{alg:vdlpn} and Algorithm~\ref{alg:vdlpn_simple}, respectively.
