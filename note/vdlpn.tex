...
We provide also an alternative low-complexity variant of the above function, that was equally presented in~\cite{vdlpn}, which we now describe in detail. We will call this variant {\sf VDLPN}$^*$. Let $w$ and $D$ be two positive integers. Both the secret key $k \in \mathbb{Z}_2^{w \times D}$ and the input $x \in \mathbb{Z}_2^{w \times D}$ are viewed as binary matrices, represented as $(k_{j,\ell})_{1 \leq j \leq w, 1 \leq \ell \leq D}$ and $(x_{j,\ell})_{1 \leq j \leq w, 1 \leq \ell \leq D}$, respectively. The function $F_K(x)$ is then computed as:
...
\bottomrule
\end{tabular}
\caption{\label{tab:vdlpn-param}Parameter sets for the {\sf VDLPN} and {\sf VDLPN}$^*$ swallow wPRFs, intended as targets for cryptanalysis.}
\end{table}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
   \State \textbf{Input:} $x = (x_{i,j,\ell}) \in \mathbb{Z}_2^{D \times w \times D}$, $k = (k_{i,j,\ell}) \in \mathbb{Z}_2^{D \times w \times D}$
    \State $y \gets 0$
    \For{$i = 1$ to $D$}
      \For{$j = 1$ to $w$}
        \State $t \gets 1$
...
\end{algorithm}
\end{minipage}
\caption{Algorithmic description of the original {\sf VDLPN} swallow wPRF and its low-complexity variant {\sf VDLPN}$^*$.}
\end{figure}
