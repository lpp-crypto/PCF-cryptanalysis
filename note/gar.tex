...
$$
\mathsf{MAJ}(x) = \mathsf{THR}_{\lceil t/2 \rceil}(x)~.
$$
...
With this criterion and the above attack in mind, it is worth noting that any predicate with locality $d = 4$ would yield an insecure PRG. Indeed, Siegenthaler~\cite{DBLP:journals/tit/Siegenthaler84} proved that for any Boolean function $f$ on $d$ variables, $\mathrm{res}(f) + \deg(f) \leq d - 1$. Consequently, when $d = 4$, one cannot simultaneously have a resiliency order greater than $2$ and an algebraic degree greater than $2$. 
This implies that the smallest viable locality is $d = 5$.  Determining whether this choice leads effectively to a secure construction remains an important theoretical question.
...
Arguably, as also discussed above, the simplest and most natural instantiation is based on the $P_5$ predicate, for which concrete parameter sets are proposed in~\cite{DBLP:journals/tit/YangGJL22} and~\cite{DBLP:conf/eurocrypt/AgarwalBBS25}.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{cccc}
    ...
    \midrule
    1024 & 1.02 & $P_5$ & \multirow{3}{*}{\cite{DBLP:journals/tit/YangGJL22}} \\
    2048 & 1.10 & $P_5$ &  \\
    4096 & 1.19 & $P_5$ &  \\
    \midrule
    8192 & 1.19 & $P_5$ & \multirow{2}{*}{\cite{DBLP:conf/eurocrypt/AgarwalBBS25}} \\
    1024 & 2 & $\mathsf{XOR}_4\textsf{-}\mathsf{MAJ}_7$ &  \\
    \midrule
    512 & 4.5 & $\mathsf{XOR}_{10}\textsf{-}\mathsf{MAJ}_{64}$ & \cite{EC:BCMPR24, EPRINT:FLLL24} \\
    \midrule
    894 & 3.4 & $\mathsf{XOR}_{10}\textsf{-}\mathsf{MAJ}_{64}$ & \multirow{2}{*}{\cite{AC:CDDKS24}} \\
    2048 & 3 & $\mathsf{XOR}_{10}\textsf{-}\mathsf{MAJ}_{64}$ &  \\
    \bottomrule
  \end{tabular}
...
\begin{algorithm}
  ...
  \State \textbf{Input:} $k\in\{0,1\}^n$, $x\in (A^n_d)^{m}$
    \State $y \gets \varepsilon$ \Comment{$\varepsilon$ denotes the empty string of length $0$}
    \For{$i = 1$ to $m$}
