...
\yr{Toute cette section est réalisée ajd vendredi 23 mai, sans vraiment grand relecture, pas convaincu de ce que j'écris, n'hésitez pas à me dire c'est nul.} \cb{J'ai relu et modifié le début de cette section, à relire.}

Twenty-five years ago, Goldreich posed the question of whether it is possible to securely construct a very simple function that is hard to invert. By \emph{very simple}, we mean a function whose description is transparent and can be expressed using a concise, closed-form formula. This question led to the design of a minimalistic and elegant one-way function. Despite its simplicity, this function is conjectured to offer strong cryptographic guarantees, and has become, over the years, a central object of study in the theory of cryptography and complexity. \cb{Add original reference}
\yr{Add: While the description is very simple, its security also relies on an access to long public random and uniform values. ?}


\subsubsection*{Original Reference}

Goldreich's proposal for constructing a one-way function is based on a very simple and elegant idea. Let  $n, m \in \mathbb{N}$, and let $(\sigma_i)_{1 \leq i \leq m}$ be a sequence of $ m $ subsets of $\{1, \dots, n\}$, each of size $d(n)$. This value $d(n)$ is referred to as the \emph{locality} of the construction. Let $ P : \{0,1\}^{d(n)} \to \{0,1\} $ be any fixed Boolean predicate (i.e., a function on $d(n)$ input bits).

For $x\in \{0,1\}^n$ and any subset $\sigma$, define $x[i]\in\{0,1\}^{d(n)}$ as $x_{\sigma(1)}||\cdots||x_{\sigma(d(n))}$ where $\sigma(j)$ is the $j$-th element in the subset $\sigma$. Then the candidate one-way function $f:\{0,1\}^n \to \{0,1\}^m$ can be expressed as the concatenation of the respective $m$ evaluations of the predicate $P$ over $x[i]$ for $i$ from $1$ to $m$. \cb{Je trouve ce dernier paragraph peu clair. Comment $x[i]$ est relié à $x$ ? Une formule explicite pour $f$ pourrait aider.}
\yr{En fait, ça donne $P(x[1])||P(x[2])||\cdots||P(x[m])$, et je ne vois pas comment faire plus simple puisqu'il y a les subsets. On en parle lundi.}


%\cb{Better description? Check if this is correct.}
%For an input $x \in \{0,1\}^n$ and a subset $\sigma \subseteq \{1, \dots, n\}$ of size $d(n)$, we define $x[\sigma] \in \{0,1\}^{d(n)} $ as the concatenation of the bits of $ x $ at the positions specified by $ \sigma $:
%\[
%x[\sigma] = x_{\sigma(1)} \| x_{\sigma(2)} \| \cdots \| x_{\sigma(d(n))},
%\]
%where $ \sigma(j) $ denotes the $ j $-th element in the ordered subset $ \sigma $.

%The candidate one-way function $ f : \{0,1\}^n \to \{0,1\}^m $ is then defined as:
%\[
%f(x) = P(x[\sigma_1]) \| P(x[\sigma_2]) \| \cdots \| P(x[\sigma_m]),
%\]
%that is, the concatenation of $ m $ applications of the predicate $ P $ to the corresponding local views of $ x $.

\yr{Ah oui ta description en commentaire est bien mieux.}

This proposal can be instantiated to yield various cryptographic primitives, such as one-way functions (when $m = n$) or pseudorandom generators with polynomial stretch (when $m = \mathcal{O}(n^{\mathsf{s}})$ for some $\mathsf{s}\geq 1$).
...
\yr{Est-ce qu'on donne le total ou est-ce qu'on garde que ça ? J'ai l'impression que c'est plus simple et lisible comme ça}

\subsubsection*{Generic cryptanalysis}
\cb{Il faut ajouter une phrase introductive a cette section.}
\yr{Oui, je suis d'accord, en fait c'est à discuter de quelle quantité on veut mettre avant, il y a des trucs qui prouvent en partie, mais je pense qu'il ne faut pas trop s'étendre dessus, à discuter}
