...
Arguably the simplest and most natural target is the instantiation using the $P_5$ predicate, for which concrete parameters are proposed in~\cite{DBLP:journals/tit/YangGJL22}. and in~\cite{DBLP:conf/eurocrypt/AgarwalBBS25}.

For an instantiation with \textsf{XOR-MAJ} predicates, some are proposed in~\cite{DBLP:conf/eurocrypt/AgarwalBBS25} and in~\cite{EC:BCMPR24} but some were broken by the bit-fixing correlation attack in~\cite{EPRINT:FLLL24} (for this ones we propose new ones) and others were proposed in~\cite{AC:CDDKS24}. All of these parameters are summarized in Table~\ref{tab:paramgar}.
\yr{Les deux paragraphes du dessus ne sont peut-être pas utiles}
...
\bottomrule
\end{tabular}
\caption{\label{tab:paramgar}Challenge parameter sets for Goldreich's PRG instantiated with the $P_5$ predicate, targeting a 128-bit security level.}
\end{table}

\subsection{Implementation}
The PRG differs from the other shallow weak PRFs presented in this survey in that it does not rely on a secret key. Instead, the adversary is given the subsets $\sigma_i$, which are sampled uniformly at random from all subsets of size $d$. These subsets play a similar role to the inputs $x$ in the other constructions: they are public, but not adversarially chosen. The actual secret in this case is the seed, which was previously denoted by $x$. For clarity and consistency with Algorithm~\ref{alg:gar}, we now refer to the seed as $k$.
