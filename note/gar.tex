...
With this criteria and attack in mind, it is worth noting that any predicate of locality 4 would directly derive an insecure PRG. Indeed, it has been shown by Siegenthaler~\cite{DBLP:journals/tit/Siegenthaler84} that for any boolean function $f$ with $d$ variables, $\mathrm{res}(f) + \deg(f) \leq d - 1$. Therefore, for $d = 4$, it is not possible to have a resiliency order bigger than $2$ and an algebraic degree bigger than $2$. The smallest achievable locality is thus $5$ and it is an important theoretic question to answer if this choice is secure or not.
...
\yr{Tried something here pour expliquer pourquoi $P_5$ ?}
