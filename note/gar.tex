...
Arguably the simplest and most natural target is the instantiation using the $P_5$ predicate, for which concrete parameters are proposed in~\cite{DBLP:journals/tit/YangGJL22}. These parameters are summarized in Table~\ref{tab:paramgar}.

\begin{table}[!ht]
\centering
\begin{tabular}{cc}
\toprule
Seed size ($n$) & stretch ($\mathsf{s}$)\\
\midrule
1024 & 1.02  \\
2048 & 1.10 \\
4096 & 1.19 \\
...
\begin{algorithm}
  ...
  \State \textbf{Input:} $k\in\{0,1\}^n$, $x\in [1,\ldots,A^n_d]^{m}$
    \State $y \gets \varepsilon$ \cb{c'est quoi $\varepsilon$?}
    \For{$i = 1$ to $m$}
